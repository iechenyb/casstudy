...
public class H2AuthDaoImpl implements UserAuthDao{
	...
	public Map<String, String> roleResources(String username) {
		String sql = "select tr.url ,tr.name,trr.roeleid,a.name "+
		" from t_resource tr,  T_ROLE_RESOURCE trr "+
		" left join t_role a  on a.id=trr.roeleid"+
	...
	public Collection<? extends GrantedAuthority> getAuthorities(String username) {
		...
		String sql = " SELECT b.name FROM T_USER_ROLE  a ,t_role b,t_user c where a.roELEID  = b.id and c.id = a.userID and c.account='"+username+"'";
	...
	public Map<String, String> roleResources() {
		...
		String sql = "SELECT b.name,b.desc,c.url "+
	  " FROM T_ROLE_RESOURCE  a,t_ROLE b,t_RESOURCE c "+
	  " where a.RESOURCEID  =c.id and b.id=a.roeleid ";
		List<Map<String,Object>> data = jdbc.queryForList(sql);
		if(!CollectionUtils.isEmpty(data)){
